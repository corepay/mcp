defmodule McpWeb.Tenant.Underwriting.KanbanLive do
  use McpWeb, :live_view

  alias Mcp.Platform.Tenant
  alias Mcp.Underwriting.Application
  alias McpWeb.Tenant.Underwriting.Components.KanbanColumn
  require Ash.Query
  ...
  def handle_event("update_status", %{"id" => id, "new_status" => new_status}, socket) do
    ...
    case Application.get_by_id(id, tenant: socket.assigns.current_tenant) do
      {:ok, application} ->
        # Update status
        Application.update!(application, %{status: status_atom},
  ...
  defp load_board(socket) do
    ...
    applications =
      Application
      # Maybe keep rejected in a separate view?
      |> Ash.Query.filter(status != :rejected)
    ...
    assign(socket, :columns, [
      %{
        ...
        title: "Leads (Draft)",
        status: :draft,
        applications: Map.get(grouped, :draft, [])
      },
      %{
        id: :submitted,
        title: "Submitted",
        status: :submitted,
        applications: Map.get(grouped, :submitted, [])
      },
      %{
        id: :under_review,
        title: "Under Review",
        status: :under_review,
        applications: Map.get(grouped, :under_review, [])
      },
      %{
        id: :manual_review,
        title: "Manual Review",
        status: :manual_review,
        applications: Map.get(grouped, :manual_review, [])
      },
      %{
        id: :approved,
        title: "Approved",
        status: :approved,
        applications: Map.get(grouped, :approved, [])
      }
    ])
  end
      ...
      <div class="px-6 py-4 border-b border-slate-800 flex justify-between items-center">
        ...
        <div class="flex space-x-2">
          <.link
            ...
            class="px-3 py-2 text-sm font-medium text-slate-400 hover:text-white"
          >
            List View
          </.link>
