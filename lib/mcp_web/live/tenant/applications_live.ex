defmodule McpWeb.Tenant.ApplicationsLive do
        ...
        if features["underwriting"] do
          ...
          applications = %{
            ...
            submitted: [
              %{
                id: "3",
                business_name: "Burger King Franchise #22",
                risk_score: 15,
                inserted_at: DateTime.utc_now()
              }
            ],
  ...
  def render(assigns) do
      ...
      <McpWeb.Core.CoreComponents.header>
        ...
        <:actions>
          <McpWeb.Core.CoreComponents.button variant="primary" size="sm">
            <.icon name="hero-plus" class="w-4 h-4 mr-2" /> New Application
    ...
    <!-- Underwriting -->
          <.kanban_column
            title="Underwriting"
            count={length(@applications.underwriting)}
            status="underwriting"
          >
  ...
  def application_card(assigns) do
    ~H"""
    <div
      class="card bg-base-100 shadow-sm hover:shadow-md transition-shadow cursor-pointer border border-base-200 group"
      phx-click={JS.navigate(~p"/tenant/applications/#{@application.id}")}
    >
