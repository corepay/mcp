defmodule McpWeb.ChatLive do
  use Elixir.McpWeb, :live_view
  # on_mount {McpWeb.LiveUserAuth, :live_user_required}
  ...
  def handle_params(%{"conversation_id" => conversation_id}, _, socket) do
    conversation =
      Mcp.Chat.get_conversation!(conversation_id, actor: socket.assigns.current_user)
    ...
    cond do
      ...
      socket.assigns[:conversation] ->
        McpWeb.Endpoint.unsubscribe("chat:messages:#{socket.assigns.conversation.id}")
        McpWeb.Endpoint.subscribe("chat:messages:#{conversation.id}")
    ...
    |> stream(:messages, Mcp.Chat.message_history!(conversation.id, stream?: true))
    |> assign_message_form()
    |> then(&{:noreply, &1})
  end
  ...
  defp assign_message_form(socket) do
    form =
      if socket.assigns.conversation do
        Mcp.Chat.form_to_create_message(
          actor: socket.assigns.current_user,
          private_arguments: %{conversation_id: socket.assigns.conversation.id}
        )
        |> to_form()
      else
        Mcp.Chat.form_to_create_message(actor: socket.assigns.current_user)
        |> to_form()
      end
