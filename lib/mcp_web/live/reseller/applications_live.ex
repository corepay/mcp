defmodule McpWeb.Reseller.ApplicationsLive do
  ...
  def mount(_params, session, socket) do
    ...
    applications = Application.read!(tenant: tenant.company_schema)
            ...
            <%= for app <- @applications do %>
              <tr class="hover">
                <td class="font-medium">
                  {app.application_data["business_name"] || app.application_data["legal_name"] ||
                    "Unknown"}
                </td>
                <td>
                  ...
                  <div class="flex gap-2">
                    <%= if app.status == :draft do %>
                      <button
                        ...
                        phx-value-id={app.id}
                      >
                        <.icon name="hero-bell-alert" class="w-4 h-4 mr-1" /> Nudge
                      </button>
                    <% end %>
                    <.link
                      navigate={~p"/partners/applications/#{app.id}"}
