defmodule Mcp.Chat.Message do
  ...
  attributes do
    ...
    attribute :source, Mcp.Chat.Message.Types.Source do
      allow_nil? false
      public? true
      default :user
    end
  ...
  relationships do
    belongs_to :conversation, Mcp.Chat.Conversation do
      public? true
      allow_nil? false
    end
  ...
  actions do
    ...
    update :respond do
      ...
      transaction? false
      change Mcp.Chat.Message.Changes.Respond
    end
  ...
  oban do
    triggers do
      trigger :respond do
        actor_persister(Mcp.AiAgentActorPersister)
        action :respond
        queue(:chat_responses)
        lock_for_update?(false)
        scheduler_cron(false)
        worker_module_name(Mcp.Chat.Message.Workers.Respond)
        scheduler_module_name(Mcp.Chat.Message.Schedulers.Respond)
