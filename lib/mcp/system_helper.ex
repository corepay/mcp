defmodule Mcp.SystemHelper do
  ...
  def get_memory_usage() do
    try do
      ...
      percentage = if total_system_memory > 0 do
        Float.round((used_memory / total_system_memory) * 100, 2)
      else
        Float.round((total_erlang / (1024 * 1024 * 1024)) * 100, 2)  # Assume 1GB if unknown
      end
      ...
      error ->
        ...
        total_erlang = Keyword.get(erlang_memory, :total, 0)
        %{
          total: 8_589_934_592,  # 8GB fallback
          used: total_erlang,
  ...
  def get_system_health() do
    ...
    Telemetry.execute([:mcp, :system, :health], %{
      cpu_usage: cpu_usage,
  ...
  defp calculate_cpu_from_reductions() do
    try do
      ...
      cpu_estimate = min(reductions / 100_000, 100.0)  # Much more conservative
      Float.round(cpu_estimate, 2)
    rescue
      _ -> 25.0  # Reasonable fallback
    end
  end
  ...
  defp get_scheduler_utilization() do
    case :scheduler.sample_all() do
      samples when is_list(samples) ->
        ...
        avg_util = total_util / length(samples) / 100.0  # Convert from percentage to 0-1 range, then to percentage
        cpu_percentage = avg_util * System.schedulers_online()
        {:ok, min(cpu_percentage, 100.0)}  # Cap at 100%
      _ -> {:error, :no_samples}
    end
  end
  ...
  defp get_total_memory_estimate() do
    ...
    cond do
      erlang_total < 100_000_000 -> 4_294_967_296      # 4GB
      erlang_total < 500_000_000 -> 8_589_934_592      # 8GB
      erlang_total < 1_000_000_000 -> 17_179_869_184   # 16GB
      true -> 34_359_738_368                            # 32GB
    end
  end

  defp get_disk_usage_command(path) do
    # Try different OS commands to get disk usage
    command_result = case :os.type() do
      {:unix, :darwin} ->  # macOS
  ...
  defp parse_df_output(output) do
    ...
    case Enum.at(lines, 1) do  # Skip header, get data line
      nil -> {:error, :no_data}
      line ->
  ...
  defp calculate_used_memory(erlang_memory) do
    ...
    total_erlang + div(total_erlang, 4)  # Add ~25% for system overhead
  end
    ...
    try do
      case :scheduler.sample_all() do
        samples when is_list(samples) ->
          ...
          load_1min = avg_util / 100  # Convert to load average format
  ...
  defp check_cpu_alerts(cpu_usage) when cpu_usage > 90 do
    [%{type: :critical, metric: :cpu, value: cpu_usage, threshold: 90, message: "Critical CPU usage"}]
  end
  ...
  defp check_memory_alerts(memory_usage) when memory_usage.percentage > 90 do
    [%{type: :critical, metric: :memory, value: memory_usage.percentage, threshold: 90, message: "Critical memory usage"}]
  end
  defp check_memory_alerts(memory_usage) when memory_usage.percentage > 80 do
    [%{type: :warning, metric: :memory, value: memory_usage.percentage, threshold: 80, message: "High memory usage"}]
  end
  ...
  defp check_load_alerts(system_load) do
    ...
    cond do
      system_load.load_1min > cpu_count * 2 ->
        [%{type: :critical, metric: :load, value: system_load.load_1min, threshold: cpu_count * 2, message: "Critical system load"}]
      system_load.load_1min > cpu_count ->
        [%{type: :warning, metric: :load, value: system_load.load_1min, threshold: cpu_count, message: "High system load"}]
      true ->
        []
    end
  end
  ...
  defp check_disk_alerts(disk_usage) do
    cond do
      disk_usage.percentage > 95 ->
        [%{type: :critical, metric: :disk, value: disk_usage.percentage, threshold: 95, message: "Critical disk usage"}]
      disk_usage.percentage > 85 ->
        [%{type: :warning, metric: :disk, value: disk_usage.percentage, threshold: 85, message: "High disk usage"}]
      true ->
        []
    end
  end
