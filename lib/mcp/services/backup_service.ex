defmodule Mcp.Services.BackupService do
  ...
  def create_tenant_backup(tenant_id, backup_options \\ %{}) do
    ...
    with {:ok, tenant} <- Tenant.get(tenant_id),
         {:ok, backup_id} <- generate_backup_id(tenant.id),
  ...
  def list_tenant_backups(tenant_id, opts \\ %{}) do
    ...
    Logger.info("Listing backups for tenant #{tenant_id} in #{backup_path}")
    ...
    if File.exists?(backup_path) do
      ...
      Logger.warning("Backup directory not found: #{backup_path}")
      {:ok, []}
    end
  rescue
    error ->
  ...
  defp filter_old_backups(backups, retention_days) do
    ...
    Enum.filter(backups, fn backup ->
      ...
      {:ok, backup_time, _} = DateTime.from_iso8601(created_at)
      DateTime.compare(backup_time, cutoff_date) == :lt
    end)
  end
  ...
  defp backup_metadata(backup_result, tenant) do
    metadata = %{
      ...
      components: %{
        database: sanitize_component(Map.get(backup_result, :database_backup)),
        files: sanitize_component(Map.get(backup_result, :file_backup)),
        configuration: sanitize_component(Map.get(backup_result, :configuration_backup))
      },
      platform_info: %{
  ...
  defp generate_backup_id(tenant_id) do
    timestamp = DateTime.to_iso8601(DateTime.utc_now())
    random_part = :crypto.strong_rand_bytes(8) |> Base.encode16()
    {:ok, "backup_#{tenant_id}_#{timestamp}_#{random_part}"}
  end
  ...
  defp get_backup_path(backup_id) do
    ...
    if length(parts) >= 4 do
      ...
      # Fallback for legacy or malformed IDs (shouldn't happen with new code)
  ...
  defp build_psql_restore_command(schema, input_path) do
    ...
    "pg_restore --host=#{hostname} --port=#{port} --username=#{username} --schema=#{schema} --clean --if-exists --no-owner --no-privileges -d #{database} #{input_path}"
  end
  ...
  defp sanitize_component(component), do: component
  ...
  defp maybe_filter_by_date_range(backups, date_from, date_to) do
    ...
    Enum.filter(backups, fn backup ->
      created_at = backup["created_at"]
      # Parse string to DateTime for comparison if needed, or compare strings if ISO8601
      # DateTime.compare requires structs.
      # Let's parse it.
      {:ok, backup_time, _} = DateTime.from_iso8601(created_at)

      from_date_check =
        if from_date, do: DateTime.compare(backup_time, from_date) != :lt, else: true

      to_date_check =
        if to_date, do: DateTime.compare(backup_time, to_date) != :gt, else: true
