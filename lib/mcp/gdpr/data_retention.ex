defmodule Mcp.Gdpr.DataRetention do
  ...
  def schedule_cleanup(user_id, expires_at, opts \\ []) do
    ...
    if is_nil(tenant_id) do
      ...
      try do
        ...
        policy_results = Enum.map(categories, fn category ->
          retention_days = calculate_retention_days(category, expires_at)

          Ash.create!(RetentionPolicy, %{
            tenant_id: tenant_id,
            entity_type: "user",
        ...
        case Enum.all?(policy_results, &(&1 != nil)) do
          true ->
            ...
            # Create audit entry
            create_audit_entry(user_id, "schedule_retention", actor_id, %{
              "categories" => categories,
  ...
  def get_user_schedules(user_id) do
    try do
      ...
      policies = Ash.read!(RetentionPolicy, action: :by_entity_type, input: %{entity_type: "user"})

      # Filter policies that apply to this specific user
      user_policies = Enum.filter(policies, fn policy ->
  ...
  def check_legal_holds(user_id) do
    try do
      ...
      active_holds = Enum.filter(user_policies, fn policy ->
        policy.legal_hold &&
          (is_nil(policy.legal_hold_until) || DateTime.compare(policy.legal_hold_until, DateTime.utc_now()) != :lt)
      end)
  ...
  def place_legal_hold(user_id, case_reference, reason, placed_by, opts \\ []) do
    ...
    if is_nil(tenant_id) do
      ...
      try do
        ...
        policies_to_hold = if categories do
          Enum.filter(existing_policies, fn policy ->
        ...
        hold_results = Enum.map(policies_to_hold, fn policy ->
          Ash.update!(policy, %{
        ...
        case Enum.all?(hold_results, &(&1 != nil)) do
          true ->
            ...
            # Create audit entry
            create_audit_entry(user_id, "place_legal_hold", placed_by, %{
  ...
  def get_retention_schedule(entity_type, tenant_id) do
    try do
      policies = Ash.read!(RetentionPolicy, action: :by_entity_type, input: %{entity_type: entity_type})

      # Filter by tenant and active policies
      tenant_policies = Enum.filter(policies, fn policy ->
  ...
  def remove_legal_hold(user_id, case_reference, released_by) do
    try do
      ...
      user_policies = get_user_schedules(user_id)
      held_policies = Enum.filter(user_policies, fn policy ->
        policy.legal_hold && String.contains?(policy.legal_hold_reason || "", case_reference)
      end)

      # Remove legal hold from each policy
      release_results = Enum.map(held_policies, fn policy ->
        Ash.update!(policy, %{actor_id: released_by},
      ...
      case Enum.all?(release_results, &(&1 != nil)) do
        true ->
          ...
          # Create audit entry
          create_audit_entry(user_id, "remove_legal_hold", released_by, %{
            "case_reference" => case_reference,
  ...
  defp calculate_retention_days(category, expires_at) do
    ...
    case category do
      "core_identity" -> max(days_until_expiry + 30, 365)  # 30-day buffer, minimum 1 year
      "transaction_data" -> max(days_until_expiry + 90, 2555)  # 90-day buffer, minimum 7 years
      "communication" -> max(days_until_expiry + 60, 1825)  # 60-day buffer, minimum 5 years
      "analytics" -> max(days_until_expiry + 30, 365)  # 30-day buffer, minimum 1 year
      _ -> max(days_until_expiry + 30, 365)  # Default 30-day buffer, minimum 1 year
    end
  end
