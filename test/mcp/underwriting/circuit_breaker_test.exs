defmodule Mcp.Underwriting.CircuitBreakerTest do
  ...
  setup do
    # Ensure CircuitBreaker is running (it should be started by app,
    # but for isolation we might want to restart or clear state if possible)
    # Since it's a named GenServer, we can just use it.
    # For robust testing, we might want to start a fresh one with a different name,
    # but the module is hardcoded to name __MODULE__.
