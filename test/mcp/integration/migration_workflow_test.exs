defmodule Mcp.Integration.MigrationWorkflowTest do
  ...
  use ExUnit.Case, async: false  # These tests may affect database state
  alias Mcp.Repo
  alias Mcp.Platform.{Tenant, DataMigration, DataMigrationLog, DataMigrationRecord}
  alias Mcp.Services.{DataMigrationEngine, DataImporter, DataTransformer, DataValidator}
  ...
  setup do
    ...
    {:ok, _} = Mcp.MultiTenant.create_tenant_schema(tenant.company_schema)
  ...
  describe "complete migration workflow" do
    test "imports customer data from CSV to tenant schema", %{tenant: tenant} do
      ...
      try do
        ...
        {:ok, migration} = DataMigration.create(%{
          tenant_id: tenant.id,
        ...
        imported_customers = MultiTenant.with_tenant_context(tenant.company_schema, fn ->
        ...
        successful_records = Enum.filter(imported_customers, fn record ->
        ...
        completion_logs = Enum.filter(logs, fn log ->
    ...
    test "handles validation errors gracefully", %{tenant: tenant} do
      ...
      try do
        ...
        assert migration_result.failed_records >= 2  # Two invalid records
        ...
        failed_records = Enum.filter(records, fn record ->
    ...
    test "applies field transformations during import", %{tenant: tenant} do
      ...
      try do
        ...
        {:ok, records} = DataMigrationRecord.by_migration(migration.id)
        successful_records = Enum.filter(records, fn record ->
          record.status == :success
        end)

        # Check first record transformations
        first_record = Enum.find(successful_records, fn record ->
    ...
    test "processes large dataset in batches", %{tenant: tenant} do
      ...
      try do
        ...
        {:ok, logs} = DataMigrationLog.by_migration(migration.id)
        batch_logs = Enum.filter(logs, fn log ->
  ...
  describe "backup and restore workflow" do
    test "creates and restores tenant backup", %{tenant: tenant} do
      ...
      # Create backup
      backup_result = Mcp.Services.BackupService.create_tenant_backup(tenant.id, %{
        "type" => "full",
        "include_files" => false
      })
      ...
      # List backups
      {:ok, backups} = Mcp.Services.BackupService.list_tenant_backups(tenant.id)
    ...
    test "cleanup old backups", %{tenant: tenant} do
      ...
      backup_ids = Enum.map(1..3, fn _i ->
        {:ok, backup_info} = Mcp.Services.BackupService.create_tenant_backup(tenant.id, %{
          "type" => "full",
          "include_files" => false
        })
      ...
      {:ok, backups_before} = Mcp.Services.BackupService.list_tenant_backups(tenant.id)
      assert length(backups_before) >= 3

      # Cleanup with 0 days retention (should delete all)
      cleanup_result = Mcp.Services.BackupService.cleanup_old_backups(tenant.id, 0)
