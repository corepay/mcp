...
- **Header-Based Only**: All internal APIs must use header-based versioning
  (e.g., `API-Version: 2024-01-01`).
- **NO Path Versioning**: Do not use `/v1/`, `/v2/` in internal API routes.

### API Response Standard

- **Success**: `%{data: <resource>}`
- **Error**:
  `%{error: %{code: <string>, message: <string>, details: <map|list>}}`
...
- **Source of Truth**: The `.env` file is the authority for port assignments.
  **DO NOT WRITE TO .ENV - READ ONLY.**
- **Verification**: Check `.env` before adding new services to ensure no
  conflicts.
- **Dynamic Ports**:
...
- If context is low, ask the user to use compact mode or create a handoff
  document.
